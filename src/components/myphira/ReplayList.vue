...
withDefaults(defineProps<{ replays: Replay[], manage?: boolean }>(), { manage: false })
...
const visibilityKey: Record<ReplayVisibility, string> = {
  inherit: 'myphira.visibilityInherit',
  ...
  custom: 'myphira.visibilityCustom',
}
...
<template>
  <p v-if="!replays.length" class="text-sm text-slate-400">
    {{ t('myphira.noReplays') }}
  </p>
    ...
    <li v-for="replay in replays" :key="replay.id" class="py-3">
      <div class="flex flex-wrap items-center gap-3">
        <div class="min-w-0 flex-1">
          ...
          <p v-if="hasValidDate(replay.created_at)" class="mt-0.5 text-xs text-slate-400">
      ...
      <details v-if="manage" class="mt-2 text-sm">
        ...
        <div class="mt-3 space-y-3 border-l border-white/10 pl-3">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <PPButton size="sm" weight="quiet" :disabled="busy !== null" @click="createShare(replay)">
              {{ t('myphira.createShare') }}
            </PPButton>
          ...
          <ul v-if="replay.share_links?.length" class="space-y-1">
            <li v-for="link in replay.share_links" :key="link.id" class="flex items-center justify-between gap-2 text-xs text-slate-400">
              ...
              <PPButton v-if="!link.revoked_at" size="sm" weight="quiet" :disabled="busy !== null" @click="revokeShare(replay, link.id)">
                {{ t('myphira.revokeShare') }}
              </PPButton>
            </li>
          </ul>
        </div>
