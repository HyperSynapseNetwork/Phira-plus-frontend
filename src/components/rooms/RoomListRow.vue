  ...
  <div class="group grid min-h-16 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 py-3">
    <button type="button" class="min-w-0 text-left focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent" @click="emit('select')">
      <div class="flex min-w-0 items-center gap-2">
        <h3 class="truncate text-sm font-medium text-[var(--pp-text-primary)] group-hover:text-accent">{{ room.name || room.room_id }}</h3>
    ...
    <div class="flex items-center gap-3 text-xs text-[var(--pp-text-secondary)]">
      ...
      <NuxtLink :to="`/room/${encodeURIComponent(room.room_id)}`" data-pp-touch-critical="room-full-page" class="pp-touch-target inline-flex h-11 w-11 items-center justify-center rounded-[var(--pp-radius-control)] text-accent hover:bg-[var(--pp-surface-2)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent" :aria-label="$t('room.openFullPage')">→</NuxtLink>
    </div>
  </div>
</template>
