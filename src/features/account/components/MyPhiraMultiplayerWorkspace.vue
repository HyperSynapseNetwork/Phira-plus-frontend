    ...
    <div class="flex items-start justify-between gap-3">
      <div>
        <h2 class="text-sm font-semibold uppercase tracking-wide text-[var(--pp-text-secondary)]">
          {{ t('myphira.tabMultiplayer') }}
        </h2>
        <p class="mt-1 text-xs text-[var(--pp-text-tertiary)]">
          {{ t('myphira.multiplayerSource') }}
        </p>
      </div>
      <PPButton weight="quiet" size="sm" :disabled="props.pending" @click="emit('refresh')">
        {{ t('common.retry') }}
      </PPButton>
    </div>
    <p v-if="props.pending" class="py-4 text-sm text-[var(--pp-text-secondary)]">
      {{ t('common.loading') }}
    </p>
    <p v-else-if="props.error" class="py-4 text-sm text-rose-300" role="alert">
      {{ t('common.error') }}
    </p>
    <template v-else-if="props.data">
      <dl class="mt-4 flex flex-wrap gap-x-8 gap-y-3 border-y border-[var(--pp-border-subtle)] py-4 text-sm">
        ...
        <div>
          <dt class="text-[var(--pp-text-tertiary)]">
            ...
            {{ formatDuration(props.data.playtime_ms) }}
          </dd>
        </div>
      </dl>
      <p v-if="props.data.recent_rounds.length === 0" class="py-5 text-sm text-[var(--pp-text-secondary)]">
        {{ t('myphira.noRounds') }}
      </p>
      <ul v-else class="divide-y divide-[var(--pp-border-subtle)]">
        <li v-for="round in props.data.recent_rounds" :key="round.round_uuid" class="grid gap-1 py-3 text-sm sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
          <div class="min-w-0">
            <p class="truncate font-medium text-[var(--pp-text-primary)]">
              {{ round.chart_name || `#${round.chart_id}` }}
            </p>
            <p class="mt-0.5 text-xs text-[var(--pp-text-tertiary)]">
              {{ t('myphira.room') }} {{ round.room_id }} · {{ formatRoundTime(round.started_at) }}
            </p>
          </div>
